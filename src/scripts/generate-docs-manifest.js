...
// Create path for the internal manifest in the same directory as the script
const scriptDir = __dirname;
const internalManifestOutputPath = path.join(scriptDir, 'atomic-docs-manifest-internal.ts');
...
// Ensure the script directory exists (it should, but just to be safe)
...
console.log(`Internal Manifest Output Path: ${internalManifestOutputPath}`);
console.log(`------------------------------------------`);
...
async function generateDocsManifest() {
  ...
  function generateManifestContent(componentsArray, examplesArray) {
    ...
    componentsArray.forEach((comp, index) => {
      componentsContent += `  {\n`;
      ...
      componentsContent += `  }${index < componentsArray.length - 1 ? ',' : ''}\n`;
    });
    componentsContent += `];\n\n`;
    ...
    examplesArray.forEach((example, index) => {
      ...
      exampleComponentsContent += `  }${index < examplesArray.length - 1 ? ',' : ''}\n`;
    });
    exampleComponentsContent += `];\n`;
...
export const exampleModules: Record<string, () => Promise<any>> = exampleComponents.reduce((acc, example) => {
  ...
  const manifestContent = generateManifestContent(allComponents, allExampleComponents);

  // Write the original manifest file
  fs.writeFileSync(manifestOutputPath, manifestContent);
  console.log(`✅ Original docs manifest generated successfully at ${manifestOutputPath}`);

  // Write the internal manifest file (duplicate) to the script directory
  fs.writeFileSync(internalManifestOutputPath, manifestContent);
  console.log(`✅ Internal docs manifest generated successfully at ${internalManifestOutputPath}`);
}
